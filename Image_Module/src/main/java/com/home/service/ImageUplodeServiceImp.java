...
public class ImageUplodeServiceImp implements ImageUplodeService {
	...
	@Override
	public ResponseEntity<ImageModelEntity> upLodeImage(MultipartFile file) throws IOException {

		System.out.println("Original Image Byte Size - " + file.getBytes().length);

		try {
			ImageModelEntity img = new ImageModelEntity(file.getOriginalFilename(), file.getContentType(),compressBytes(file.getBytes()));
			return ResponseEntity.ok(imageRepository.save(img));
			
		}catch(ResponseStatusException e) {
			return new ResponseEntity<ImageModelEntity>(e.getStatus());
		}
